...
- **压缩极限反应堆**（`compact_reactor`）：燃料棒、控制棒、功率接口数量与内部尺寸可通过配置调整；能量每游戏刻向 6 个相邻方向主动推送（模拟 ActivePowerTapFE），并按被动模式等效公式补记 FE——以模拟满容量输出（默认配置约 3.7k FE/t）。水/蒸汽流体接口已接通，但蒸汽转化路径暂未生效（见下方已知限制）；
- **压缩涡轮机**（`compact_turbine`）：转子/线圈规模由模拟布局与配置决定；蒸汽可经流体接口注入并储存，但转子尚未发电（ER2 基于部件的转子启动机制未纳入模拟，属路线图事项）；
...
- 压缩极限反应堆当前**仅输出 FE**——水→蒸汽转化与压缩涡轮机的发电依赖 ER2 基于真实部件的内部机制（蒸发访问检查、转子启动），单方块模拟暂未覆盖；蒸汽可正常注入并储存于流体罐。蒸汽支持已列入路线图。
...
| `reactor.sizeX/Y/Z` | 9 | 模拟反应堆尺寸（3–64，能量缓冲随体积缩放） |
