...
- **压缩反应堆**（`compact_reactor`）：燃料棒、控制棒、功率接口数量与内部尺寸可通过配置调整；能量每游戏刻向 6 个相邻方向主动推送（模拟 ActivePowerTapFE），并按被动模式等效公式补记 FE——以模拟满容量输出（默认配置约 3.7k FE/t）。水/蒸汽流体接口已接通，但蒸汽转化路径暂未生效（见下方已知限制）；
...
- 压缩反应堆当前**仅输出 FE**——水→蒸汽转化与压缩涡轮机的发电依赖 ER2 基于真实部件的内部机制（蒸发访问检查、转子启动），单方块模拟暂未覆盖；蒸汽可正常注入并储存于流体罐。蒸汽支持已列入路线图。
