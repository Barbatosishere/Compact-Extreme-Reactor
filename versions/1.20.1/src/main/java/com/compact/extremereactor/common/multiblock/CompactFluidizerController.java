...
public class CompactFluidizerController
    ...
    public void syncDataFrom(CompoundTag tag, ISyncableEntity.SyncReason reason) {
        ...
        this.syncChildDataEntityFrom(this._solidMixingHolder, NBT_RECIPE_SOLID_MIXING, tag, reason);
        this._fluidMixingHolder.refresh();
        this.syncChildDataEntityFrom(this._fluidMixingHolder, NBT_RECIPE_FLUID_MIXING, tag, reason);
    ...
    public CompoundTag syncDataTo(CompoundTag tag, ISyncableEntity.SyncReason reason) {
        tag = super.syncDataTo(tag, reason);

        this.syncBooleanElementTo(NBT_ACTIVE, tag, this.isMachineActive());
        this.syncChildDataEntityTo(this._energyBuffer, NBT_ENERGY, tag, reason);
        this.syncChildDataEntityTo(this._outputTank, NBT_OUTPUT, tag, reason);
        this.syncChildDataEntityTo(this._itemInputs, NBT_ITEMS, tag, reason);
        this.syncChildDataEntityTo(this._fluidInputs0, NBT_FLUID_IN_0, tag, reason);
        this.syncChildDataEntityTo(this._fluidInputs1, NBT_FLUID_IN_1, tag, reason);
        this.syncChildDataEntityTo(this._solidHolder, NBT_RECIPE_SOLID, tag, reason);
        this.syncChildDataEntityTo(this._solidMixingHolder, NBT_RECIPE_SOLID_MIXING, tag, reason);
        this.syncChildDataEntityTo(this._fluidMixingHolder, NBT_RECIPE_FLUID_MIXING, tag, reason);
