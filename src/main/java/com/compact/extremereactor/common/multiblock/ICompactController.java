...
public interface ICompactController extends IWideEnergyStorage2 {
    ...
    /** 从 NBT 恢复控制器数据。 */
    void syncDataFrom(CompoundTag tag, HolderLookup.Provider registries, ISyncableEntity.SyncReason reason);

    /** 将控制器数据写入 NBT。 */
    CompoundTag syncDataTo(CompoundTag tag, HolderLookup.Provider registries, ISyncableEntity.SyncReason reason);
